...
export async function inviteMember(fd: FormData): Promise<ActionResult> {
  try {
    ...
    if (!["admin", "staff"].includes(role))  return { ok: false, error: "Rol inválido" };
    ...
    if (!canInvite) return { ok: false, error: "Alcanzaste el límite de miembros de tu plan. Actualizá para agregar más." };
...
export async function removeMember(targetId: string): Promise<ActionResult> {
  try {
    ...
    if (targetId === user.id) return { ok: false, error: "No podés eliminarte a vos mismo" };
...
export async function updateMemberRole(targetId: string, role: string): Promise<ActionResult> {
  try {
    ...
    if (!["admin", "staff"].includes(role)) return { ok: false, error: "Rol inválido" };
    if (targetId === user.id) return { ok: false, error: "No podés cambiar tu propio rol" };
