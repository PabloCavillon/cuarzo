...
import { POST, GET } from "@/app/api/turnera/bookings/route";
...
function postReq(body: unknown): NextRequest {
  ...
  return new NextRequest("http://localhost/api/turnera/bookings", {
    method:  "POST",
...
function getReq(params: Record<string, string>): NextRequest {
  const url = new URL("http://localhost/api/turnera/bookings");
...
describe("POST /api/turnera/bookings", () => {
  beforeEach(() => {
    vi.resetAllMocks();
  ...
  it("returns 413 for oversized body", async () => {
    const bigReq = new NextRequest("http://localhost/api/turnera/bookings", {
      method:  "POST",
  ...
  it("returns 422 for invalid JSON body", async () => {
    const badReq = new NextRequest("http://localhost/api/turnera/bookings", {
      method:  "POST",
...
describe("GET /api/turnera/bookings", () => {
  beforeEach(() => {
    vi.resetAllMocks();
    prismaMock.tenant.findUnique.mockResolvedValue(DEMO_TENANT);
  });
