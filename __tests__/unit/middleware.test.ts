...
describe("rate limiter — isAllowed", () => {
  ...
  it("passes the first request for a rate-limited route", () => {
    const req = makeReq("POST", "/api/turnera/bookings");
    const res = middleware(req as never);
    expect((res as NextResponse).status).not.toBe(429);
  });

  it("passes up to the limit and blocks the next one (POST /api/turnera/bookings → 5 req/min)", () => {
    const ip = uniqueIp();
    const path = "/api/turnera/bookings";
  ...
  it("GET and POST share different quota keys (different route keys)", () => {
    const ip = uniqueIp();
    // GET /api/turnera/bookings → 20 req/min
    // POST /api/turnera/bookings → 5 req/min

    // Exhaust POST quota
    for (let i = 0; i < 5; i++) middleware(makeReq("POST", "/api/turnera/bookings", ip) as never);
    const blockedPost = middleware(makeReq("POST", "/api/turnera/bookings", ip) as never) as NextResponse;
    expect(blockedPost.status).toBe(429);

    // GET quota is independent — first GET should pass
    const allowedGet = middleware(makeReq("GET", "/api/turnera/bookings", ip) as never) as NextResponse;
  ...
  it("GET /api/turnera/services allows 60 req/min", () => {
    const ip = uniqueIp();
    for (let i = 0; i < 60; i++) {
      const res = middleware(makeReq("GET", "/api/turnera/services", ip) as never) as NextResponse;
      expect(res.status).not.toBe(429);
    }
    const blocked = middleware(makeReq("GET", "/api/turnera/services", ip) as never) as NextResponse;
    expect(blocked.status).toBe(429);
  });
});
