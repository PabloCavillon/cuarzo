...
export async function updateProvider(id: string, formData: FormData): Promise<ActionResult> {
  ...
  if (!provider || provider.tenantId !== user.tenantId) return { ok: false, error: "Provider not found." };
...
export async function toggleProvider(id: string, active: boolean): Promise<ActionResult> {
  let user;
  try { user = await requireAuth("staff"); } catch { return { ok: false, error: "Unauthorized." }; }
