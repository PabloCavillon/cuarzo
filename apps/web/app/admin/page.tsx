...
export default async function AdminDashboard() {
  ...
  return (
    <div className="max-w-5xl mx-auto space-y-7">
      ...
      {/* KPI row 1 — Bookings */}
      <div>
        <p className="text-[10px] font-semibold text-white/25 uppercase tracking-widest mb-3">Appointments</p>
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
          <StatCard label="Appointments today"       value={todayBookings}          sub="Confirmed"     Icon={Calendar}      href="/admin/appointments" />
          <StatCard label="This week"       value={weekBookings}           sub="Since Monday"  Icon={Clock}         href="/admin/appointments" />
          <StatCard label="Active services" value={activeServices}         sub="In scheduler"      Icon={CheckCircle2}  href="/admin/appointments/services" />
          <StatCard label="Upcoming"          value={upcomingBookings.length} sub="Confirmed"    Icon={Calendar}      href="/admin/appointments" />
        </div>
      </div>
      ...
      <div>
        ...
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
          <StatCard label="Active customers"  value={clientCount}   sub="CRM base"        Icon={Users}   href="/admin/customers" />
          <StatCard label="Providers"        value={supplierCount} sub="Active"         Icon={Users}   href="/admin/providers" />
          <StatCard label="Products"          value={productCount}  sub="In catalog"     Icon={Package} href="/admin/catalog" />
          <StatCard
            label="Low stock"
            value={lowStockCount}
            sub="≤ 5 units"
        ...
        <div className="grid grid-cols-3 gap-4">
          <div className="bg-white/5 border border-white/8 rounded-2xl p-5">
            <div className="flex items-center gap-2 mb-3">
              ...
              <p className="text-xs text-white/40 font-medium uppercase tracking-wide">Revenue</p>
            </div>
          ...
          <div className="bg-white/5 border border-white/8 rounded-2xl p-5">
            <div className="flex items-center gap-2 mb-3">
              ...
              <p className="text-xs text-white/40 font-medium uppercase tracking-wide">Expenses</p>
            </div>
            <p className="text-2xl font-bold text-red-400">$ {fmt(cajaEgresos)}</p>
          </div>
          <Link href="/admin/finance" className="group bg-white/5 border border-white/8 rounded-2xl p-5 hover:bg-white/8 hover:border-white/12 transition-colors block">
            <div className="flex items-center gap-2 mb-3">
              <Wallet className="w-4 h-4 text-white/40" />
              <p className="text-xs text-white/40 font-medium uppercase tracking-wide">Balance</p>
            </div>
      ...
      <div className="grid lg:grid-cols-2 gap-6">
        ...
        <div className="bg-white/5 border border-white/8 rounded-2xl p-5">
          <div className="flex items-center justify-between mb-4">
            ...
            <Link href="/admin/appointments" className="text-xs text-white/40 hover:text-white transition-colors">
              Ver todos →
            </Link>
          </div>
