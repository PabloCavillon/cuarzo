...
export default function LoginForm() {
  ...
  return (
    <form onSubmit={handleCredentials} className="flex flex-col gap-4">
      <h1 className="text-xl font-semibold text-white text-center mb-2">{tl.title}</h1>
      {error && (
      ...
      <div className="flex flex-col gap-1">
        <label className="text-xs text-white/60 font-medium" htmlFor="email">
          {tl.emailLabel}
        </label>
        <input
          id="email" type="email" autoComplete="email" required maxLength={254}
          value={email} onChange={(e) => setEmail(e.target.value)}
          placeholder={tl.emailPlaceholder} className={inputCls}
        />
      </div>

      <div className="flex flex-col gap-1">
        <div className="flex items-center justify-between">
          <label className="text-xs text-white/60 font-medium" htmlFor="password">
            {tl.passwordLabel}
          </label>
        ...
        <input
          ...
          placeholder={tl.passwordPlaceholder} className={inputCls}
        />
      </div>
